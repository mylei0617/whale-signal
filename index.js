...
app.get('/debug-tg', async (req, res) => {
  ...
  if (BOT_TOKEN && CHAT_ID) {
    try {
      ...
      tgTest = j.ok ? 'SENT OK msg_id=' + j.message_id : 'FAIL: ' + JSON.stringify(j);
    } catch(e) {
      tgTest = 'ERROR: ' + e.message;
    }
  } else {
    tgTest = 'SKIPPED (missing token or chat_id)';
  }
  
  console.log('[debug] env vars:', JSON.stringify(allVars));
...
(function() {
  ...
  if (BOT && CHAT) {
    fetch('https://api.telegram.org/bot' + BOT + '/sendMessage', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ chat_id: CHAT, text: '*Railway started* - Bot is running!', parse_mode: 'Markdown' })
    }).then(function(r) { return r.json(); })
      .then(function(j) { console.log('[init] Telegram:', j.ok ? 'SENT msg_id='+j.message_id : 'FAIL'); })
